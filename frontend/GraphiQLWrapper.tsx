...
function GraphiQLWrapper(props: {
  ...
  const hideHeaderValue = (header: { key: string; value: string }) => {
    return (
      filter(
        ["auth", "token", "secret"],
        (keyword) =>
          header.key.toLocaleLowerCase().includes(keyword) ||
          header.value.toLocaleLowerCase().includes(keyword)
      ).length >= 1
    );
  };
  ...
  return (
    <div>
      <Box paddingBottom={"16px"}>
        <Heading as="h6">GraphQL Endpoint</Heading>
      ...
      <Box paddingBottom={"16px"}>
        <Heading as="h6">Request Headers</Heading>
          ...
          return (
            ...
            >
              <Input
                ...
                autoFocus={
                  index === props.headers.length - 1 && header.key == ""
                }
                onChange={(e) => {
                  const newHeaders = cloneDeep(props.headers);
              ...
              <Button
                ...
                onClick={() => {
                  const newHeaders = cloneDeep(props.headers);
                  props.setHeaders(
                    remove(newHeaders, (_, _index) => {
                      return index !== _index;
                    })
                  );
                }}
              >
                <Icon style={{ marginTop: "4px" }} name="x"></Icon>
              </Button>
            </Box>
          );
        })}
      ...
      <Box>
        ...
        <div className="graphiql-container" style={{ height: "50vh" }}>
          <GraphiQL
            ...
            onEditQuery={(query) => {
              // TODO debounce & save for import dialog
            }}
          />
        </div>
      </Box>
